        ...
        if (transform.position.x < generationPoint.position.x)
        {
            ...
            if (Random.Range(0f, 100f) < randomFireThreshold)
            {
                if (firePool)
                {
                    ...
                    float fireXPosition = Random.Range(-platformWidths[platformSelector] / 2f + 1f, platformWidths[platformSelector] / 2f - 1f);


                    Vector3 firePosition = new Vector3(fireXPosition, 0.5f, 0f);
